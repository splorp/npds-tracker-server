...
```kPort``` — Specifies the TCP listening port.
...
Example specifying multiple ports:

```sh
kPort = 3680 8080
```

```trackerName``` — Specifies the name of the site or tracker for use in the HTML template.
...
```trackerHost``` — Specifies the URL of the site or server host for use in the HTML template.
...
```adminPass``` — Specifies the password for the remote administration interface.
...
```validateTime``` — Specifies how often to validate clients.
...
```validateTries``` — Specifies how many validation attempts before clients are removed from the list.
...
```shshareEnabled``` — Specifies whether other trackers may obtain SHARE’d records from this tracker.
...
```pageTemplate``` — Specifies the path and name of the HTML template.
...
```cssTemplate``` — Specifies the path and name of the template stylesheet.
...
```shareServer``` — Specifies servers to obtain SHARE’d records from.
...
```logVerbose``` — Specifies whether messages should be written to the log file.
...
```privateHostToAccept``` — Specifies a private hostname or address to register as a client.
