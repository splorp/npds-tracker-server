...
Log entries generated by the tracker are rather verbose and the log file can grow quite large over time. You can turn off logging altogether by commenting out the ```logfile``` setting once you are sure that the server is configured properly.

You can also modify the list of tracker servers that you want to share records with by commenting out or adding ```shareServer``` settings.
